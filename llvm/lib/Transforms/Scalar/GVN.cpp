  ...
  for (const auto &AvailableLoad : AvailableLoads) {
    ...
    if (CriticalEdgePredAndLoad) {
      ...
      if (I != CriticalEdgePredAndLoad->end()) {
        ...
        // The dead OldLoad will be deleted when we process
        // OldLoad->getParent().
        AdditionalWorkSet.insert(OldLoad->getParent());
      }
    }
  }
...
bool GVNPass::iterateOnFunction(Function &F) {
  ...
  for (BasicBlock *BB : RPOT) {
    AdditionalWorkSet.erase(BB);
    Changed |= processBlock(BB);
  }

  for (BasicBlock *BB : AdditionalWorkSet)
    Changed |= processBlock(BB);
